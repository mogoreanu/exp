/*
bazel test stat:throughput_counter_test --test_output=streamed
*/
...
// Interval size: 8388608 Interval count: 119 leftover:0.20928955078125
// using MyThroughputCouter = ThroughputCounter<8388608, 120>;

constexpr int64_t NsFromMs(int64_t ms) { return ms * 1000000LL; }
constexpr int64_t NsFromS(int64_t s) { return s * 1000000000LL; }

TEST(ThroughputCounterTest, BasicAligned) {
  // 10 spans, 100ms each.
  ThroughputCounter<NsFromMs(100), 10> c1;
  int64_t now_ns = NsFromS(100);
  c1.Record(100, now_ns, now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(100, c1.GetThroughput(now_ns));

  c1.Record(100, now_ns - NsFromMs(100), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(200, c1.GetThroughput(now_ns));

  c1.Record(100, now_ns - NsFromMs(200), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(300, c1.GetThroughput(now_ns));

  c1.Record(100, now_ns - NsFromMs(300), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(400, c1.GetThroughput(now_ns));

  c1.Record(100, now_ns - NsFromMs(400), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(500, c1.GetThroughput(now_ns));

  c1.Record(100, now_ns - NsFromMs(2000), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(600, c1.GetThroughput(now_ns));
}

TEST(ThroughputCounterTest, BasicUnaligned) {
  // 10 spans, 100ms each.
  ThroughputCounter<NsFromMs(100), 10> c1;
  int64_t now_ns = NsFromS(100) + NsFromMs(50);
  c1.Record(100, now_ns, now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(100, c1.GetThroughput(now_ns));
  ...
  c1.Record(100, now_ns - NsFromMs(2000), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(597, c1.GetThroughput(now_ns));
}

TEST(ThroughputCounterTest, BasicQarter) {
  // 10 spans, 100ms each.
  ThroughputCounter<NsFromMs(100), 10> c1;
  int64_t now_ns = NsFromS(100) + NsFromMs(25);
  c1.Record(100, now_ns, now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(100, c1.GetThroughput(now_ns));
  ...
  c1.Record(100, now_ns - NsFromMs(2000), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(598, c1.GetThroughput(now_ns));
}

TEST(ThroughputCounterTest, BasicDecay) {
  // 10 spans, 100ms each.
  ThroughputCounter<NsFromMs(100), 10> c1;
  int64_t now_ns = NsFromS(100);
  c1.Record(1000, now_ns - NsFromMs(2000), now_ns);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(1000, c1.GetThroughput(now_ns));

  now_ns += NsFromMs(100);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(900, c1.GetThroughput(now_ns));

  now_ns += NsFromMs(100);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(800, c1.GetThroughput(now_ns));

  now_ns += NsFromMs(100);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(700, c1.GetThroughput(now_ns));
  ...
  now_ns += NsFromMs(100);
  LOG(INFO) << c1.ToDebugString(now_ns);
  ASSERT_EQ(00, c1.GetThroughput(now_ns));
}
...
//   LOG(INFO) << "BPS at start = " <<
//   counter_.GetBytesPerInterval(current_time_); absl::Duration interval =
//   GetParam(); new (&counter_) ApproxCounter(current_time_, interval); const
//   std::vector<int64_t> throughputs_to_test = {
//       10 * kMegabyte,  40 * kMegabyte,  120 * kMegabyte,  240 * kMegabyte,
//       400 * kMegabyte, 800 * kMegabyte, 1200 * kMegabyte, 2000 * kMegabyte};
...
// /* This test can be used to display the accuracy of the
// ApproxThroughputCouter. approx=41 timeseries=23 actual=41 approx=41
// timeseries=47 actual=41 approx=41 timeseries=35 actual=41 approx=41
// timeseries=46 actual=41 approx=41 timeseries=39 actual=41
