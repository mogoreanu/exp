...
absl::Duration BurstTokenBucket::TryGetTokens(absl::Time now,
  ...
  // absl::Duration refill_tokens = max_burst_tokens_ - cur_burst_tokens_;
  absl::Duration delay = tb_.TryGetTokens(now - max_burst_tokens_, tokens);
  if (delay == absl::ZeroDuration()) {
    return absl::ZeroDuration();
  }
  if (delay <= max_burst_tokens_) {
    delay = tb_.TryGetTokens(now - max_burst_tokens_ + delay, tokens);
