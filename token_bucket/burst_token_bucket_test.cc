...
TEST(RateTokenBucketTest, Test50s) {
  ...
  LOG(INFO) << "Just constructed: " << btb << " now: " << now;

  for (int i = 0; i < 3; ++i) {
    // Drain the burst, all should go through.
    ASSERT_EQ(absl::ZeroDuration(), btb.TryGetTokens(now, token));
    LOG(INFO) << "After extracted 1 : " << btb << " now: " << now;
    ASSERT_EQ(absl::ZeroDuration(), btb.TryGetTokens(now, token));
    LOG(INFO) << "After extracted 2 : " << btb << " now: " << now;
    ASSERT_EQ(absl::ZeroDuration(), btb.TryGetTokens(now, token));
    LOG(INFO) << "After extracted 3 : " << btb << " now: " << now;
    ...
    ASSERT_EQ(absl::ZeroDuration(),
              btb.TryGetTokens(now, negative_bucket_value));
    LOG(INFO) << "After extracted 4 : " << btb << " now: " << now;
    ...
    // We don't move the time, nothing changes.
    ASSERT_EQ(negative_bucket_value, btb.TryGetTokens(now, token));
  ...
  while (now < end_time) {
    absl::Duration d = btb.TryGetTokens(now, absl::Microseconds(100));
  ...
  ASSERT_NEAR(total_rate, 10000, /*abs_error=*/1);
}
